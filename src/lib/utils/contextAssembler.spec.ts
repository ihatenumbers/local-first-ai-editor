...
describe('contextAssembler', () => {
	...
	it('builds system prompt for lints', () => {
		const scene = {
			...
			todoList: [
				{ id: 't1', text: 'Fix first paragraph', status: 'open', source: 'user', createdAt: 123 }
			],
			reviewRecipes: [],
		...
		expect(prompt).toContain(
			'CRITICAL: You must output YOUR ENTIRE RESPONSE as a valid JSON object containing a "response" array of objects'
		);
	...
	it('builds system prompt for todos', () => {
		...
		const prompt = buildSystemPrompt(scene, mockProject, recipe);
		expect(prompt).toContain(
			'CRITICAL: You must output YOUR ENTIRE RESPONSE as a valid JSON object containing a "response" array of strings.'
		);
		expect(prompt).toContain('### INSTRUCTIONS (YOUR PRIMARY TASK)\nSuggest tasks.');
	});
