<script lang="ts">
	...
	let showReviewRecipes = $state(true);
	let showContextBoard = $state(true);

	// Mock Data for Header
	let wordCount = $state(1245);
	let todoCount = $state(3);
	let currentChapter = $state(1);
	let currentScene = $state(2);
</script>
		...
		<aside class="w-64 flex-col border-r border-zinc-200 bg-zinc-100/50 flex shrink-0 transition-all">
			...
			<div class="flex-1 overflow-y-auto p-4">
				<div class="text-sm text-zinc-600 font-medium cursor-pointer hover:text-zinc-900">
					CH {currentChapter}
					<div class="ml-4 mt-1 text-zinc-500 font-normal">SC {currentScene}</div>
				</div>
			</div>
		</aside>
	{/if}
	...
	<section class="flex flex-1 flex-col overflow-hidden bg-white relative shadow-sm z-10 min-w-[400px]">
		...
		<header class="sticky top-0 flex items-center justify-between border-b border-zinc-200 bg-white px-4 py-2 z-20 text-xs text-zinc-600">
			...
			<div class="flex items-center font-mono font-medium bg-zinc-100 px-2 py-1 rounded">
				CH {currentChapter} <span class="mx-2 text-zinc-300">/</span> SC {currentScene}
			</div>

			<div class="flex items-center gap-6">
				<div class="flex items-center gap-4">
					<span>{wordCount} Words</span>
					<span class="flex items-center gap-1"><CheckSquare size={14} /> {todoCount} To-Dos</span>
				</div>
			...
			<div class="flex items-center gap-1">
				<button 
					class="p-1.5 rounded hover:bg-zinc-100 transition-colors {showExplorer ? 'text-indigo-600' : 'text-zinc-400'}"
				...
				<button 
					class="p-1.5 rounded hover:bg-zinc-100 transition-colors {showReviewRecipes ? 'text-indigo-600' : 'text-zinc-400'}"
					onclick={() => showReviewRecipes = !showReviewRecipes}
					title="Toggle Review Recipes & To-Dos">
					<ListChecks size={18} />
				</button>
				
				<button 
					class="p-1.5 rounded hover:bg-zinc-100 transition-colors {showContextBoard ? 'text-indigo-600' : 'text-zinc-400'}"
					onclick={() => showContextBoard = !showContextBoard}
			...
			<div class="flex-1 overflow-y-auto p-4 space-y-6">
				...
				<div>
					<div class="flex items-center justify-between mb-2">
						...
						<span class="text-xs font-mono bg-zinc-200 px-1.5 py-0.5 rounded text-zinc-600">{todoCount}</span>
					</div>
